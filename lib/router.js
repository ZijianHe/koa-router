...
router.middleware = function() {
  ...
  return function *dispatch(next) {
    ...
    if (matchedRoutes = router.match(pathname)) {
      ...
      for (var len = matchedRoutes.length, i=0; i<len; i++) {
        ...
        for (var l = route.methods.length, n=0; n<l; n++) {
          ...
          if (method === this.method) {
            ...
            this.captures = matched.captures;
      ...
      if (this.method === 'HEAD' && methodsAvailable.GET) {
        merge(this.params, matched.params);
        return yield *matched.route.middleware.call(this, next);
      }
...
router.match = function(path) {
  ...
  for (var len = routes.length, i=0; i<len; i++) {
    ...
    if (matched = routes[i].match(path)) {
