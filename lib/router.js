...
router.match = function(path) {
  ...
  for (var len = routes.length, i=0; i<len; i++) {
    ...
    if (params && routerValidates(routes[i], params)) {
...
function routerValidates(route, params) {
  ...
  for (var name in params) {
    if (typeof route.validate[name] != 'undefined') {
      var regexPattern = new RegExp(route.validate[name]);
      if (params[name].match(regexPattern)) {
        continue;
      }
