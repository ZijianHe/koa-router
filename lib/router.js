...
/**
 * Register a validator against a named route
 *
 * @param {String} name.
 * @param {Object} Object with regexes
 * @return {Route}
...
router.validate = function(name, requirements) {
  var route = this.route(name);

  if (route) {
    route.validate = requirements;
  }
...
router.match = function(path) {
  ...
  if (typeof this.allowExtensionsValidator !== 'undefined') {
    ...
    if (this.allowNoExtension && ext === false) {
      ...
      var regexPattern = validate instanceof RegExp ? validate : new RegExp(validate);

      if (!ext || !ext.match(regexPattern)) {
        return false;
      }
    }
  }
  ...
  for (var len = routes.length, i=0; i<len; i++) {
    ...
    if (params) {
      params.ext = ext;
      if (routerValidates(routes[i], params)) {
...
router.extendApp = function(app) {
  ['all', 'redirect', 'register', 'url', 'del', 'allowExtensions'].concat(methods)
...
function routerValidates(route, params) {
  if (typeof route.validate !== 'object') {
    return true;
  }

  for (var name in params) {
    var validate = route.validate[name]
      , param = params[name];
