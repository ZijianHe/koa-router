...
router.middleware = function() {
  ...
  if (router._middleware.length) {
    return compose(router._middleware.concat(middleware));
  } else {
    return middleware;
  }
};
 ...
 * app.use(router.use(session(), authorize()).middleware());
 * ```
 *
...
router.use = function (middleware) {
  this._middleware.push.apply(this._middleware, arguments);
  return this;
};
