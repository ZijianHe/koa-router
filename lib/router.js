...
class Router {
  ...
  static path(path, ...args) {
    return new Route({ path }).toPathWithParams(...args);
  }
...
domain, and/or subdomain in the returned string.`,
  ...
  allowedMethods({
    ...
    return compose([
      Router.createNotImplementedHandler({
        throw: _throw,
        implementedMethods: this.methods,
  ...
  compile({ prefix = '' } = {}) {
    debug(
      'compiling router name=%s prefix=%s',
      this.name || 'anonymous',
      prefix,
    );
    return this.stack.reduce((accumulator, member) => {
      if (member.compile) {
        return accumulator.concat(member.compile({ prefix: `${prefix}${this.prefix}` }));
      } else {
        return accumulator.concat(member);
      }
    }, []);
  }
  ...
  createDispatch() {
    ...
    const dispatch = (ctx, next) => {
      ...
      ctx.matched = stack
        .filter(member => member.route && member.route.terminal && member.route.test(path))
        .map(member => member.route);
      const match = ctx.matched.find(route => route.handles(method));
  ...
  static createNotImplementedHandler({ throw: canThrow, errorFactory, implementedMethods }) {
    const notImplementedHandler = (ctx, next) => {
      ...
      if (implementedMethods.has(method) || method === 'OPTIONS') return next();
      ...
      if (canThrow) {
        ...
        ctx.status = 501;
        ctx.set('Allow', Array.from(implementedMethods).join(', '));
        return null;
      }
    };
