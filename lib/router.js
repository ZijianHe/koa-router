...
class Router {
  ...
  static url(...args) {
    return withDeprecationWarning(
      () => this.path(...args),
      `.url behavior will change in version 9.
Please use .path if you only need the URL path and not the host, domain, and/or subdomain in the returned string.
...
Please use Router#path if you only need the URL path and not the host, domain, and/or subdomain in the returned string.
Called from: ${new Error().stack.split('\n').slice(1, 15).join('\n')}
`,
    );
  }
   ...
   *
   * @param {object} options
   * @param {(boolean|string)} [options.allowedMethods=false] add allowedMethods and notImplemented
   *                                                 middleware to the returned routing function
   *                                                 when `true`, the middleware is added and
   *                                                 405, 501, and OPTIONS responses are handled
   *                                                 for you. When `'throw'`, 405 and 501 responses
   *                                                 are handled by throwing an error
    ...
    if (allowedMethods) {
      stack.push(
        this.createNotImplementedHandler({
          ...
          errorFactory: notImplemented || (() => new errors.NotImplemented()),
        }),
        Router.createAllowedMethodsHandler({
          throw: allowedMethods === 'throw',
          errorFactory: methodNotAllowed || (() => new errors.MethodNotAllowed()),
        }),
      );
    }
   ...
   *
   * available for backwards compatibility < 8.x
   *
   * @private
   */
