...
Router.prototype.routes = Router.prototype.middleware = function () {
  ...
  return function *dispatch(next) {
    ...
    if (route) {
      ...
      this.route = route;
      this.forward = function *(url, next) {
        return yield router.forward(this, url, method, next);
      };
...
/**
 * Forward route and parameters to given url (or named route)
 *
 * @param {String} url
 * @param {Function} next
 * @returns {Route}
 */
Router.prototype.forward = function *(ctx, url, method, next) {
  var matched = this.match(url);
  var route = matched.filter(function (route) {
    return ~route.methods.indexOf(method);
  }).shift();
  if (!route) {
    throw new HttpError('Route ' + url + ' not found');
  }
  return yield *route.middleware.call(ctx, next);
};
