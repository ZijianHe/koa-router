...
router.advice = function(name, middleware) {
  var advices = this[name];
  if(!advices) return;
  if(!~advices.indexOf(middleware)) {
    advices.push(middleware);
  }
  return this;
};
...
router.middleware = function() {
  ...
  return function *dispatch(next) {
    ...
    this.routesMatched = this.routesMatched || [];
      ...
      for (var len = matchedRoutes.length, i=0; i<len; i++) {
        ...
        for (var l = route.methods.length, n=0; n<l; n++) {
          ...
          if (method === this.method) {
            ...
            this.status = 200;//reset to 200
            this.routesMatched.push(route);
            if(router.after.length || router.before.length) {
              return yield compose(
                router.before.concat(route.middleware, router.after)
              ).call(this, next)
            }
